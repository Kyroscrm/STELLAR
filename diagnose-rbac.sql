-- Diagnostic Script for RBAC Issues
-- Run this in Supabase SQL Editor to check what's wrong

-- Check 1: Verify admin user exists in profiles
SELECT 'Admin User Check' as check_name,
       id, role_id, full_name, email
...
-- Check 4: Verify role_permissions connections exist
SELECT 'Role Permissions Check' as check_name,
       COUNT(*) as connection_count
FROM public.role_permissions;

-- Check 5: Detailed check of admin role and permissions
...
-- Check 6: Verify the specific user's role assignment
...
-- Check 7: Test the exact query that useRBAC hook uses
...
-- Check 8: Test permissions query for admin role
WITH user_role AS (
...
SELECT 'Summary' as check_name,
       'Data exists and should work' as result;
